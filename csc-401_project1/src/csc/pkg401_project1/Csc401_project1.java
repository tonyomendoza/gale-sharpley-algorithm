...
public class Csc401_project1 {
	...
	static void printMatches(int startingMan) {
                System.out.println("\t=>M" + startingMan + " proposes. It takes " + proposalCount + " proposals to reach the final stable marriage.");
		...
		for (int i = 0; i < men.size(); i++) {
                    men.get(i).setPartner(-1);
                    women.get(i).setPartner(-1);
		}
                proposalCount = 0;
        }
}